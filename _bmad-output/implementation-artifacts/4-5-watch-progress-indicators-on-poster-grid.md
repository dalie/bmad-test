...
- [ ] 1. Define localStorage schema interfaces in `apps/frontend/src/app/home/home.component.ts`
  - [ ] 1.1 Export `WatchProgressEntry` interface — the per-file progress record stored in localStorage:
    ```typescript
    ...
    ```
  - [ ] 1.2 Export `WatchProgressRecord` type — the top-level JSON stored at `WATCH_PROGRESS_KEY`:
    ```typescript
    // Key format:
  ...
  - [ ] 1.3 Export `ContinueWatchingItem` interface extending `LibraryItem`:
    ```typescript
...
- [ ] 2. Implement `buildProgressData()` and update `readContinueWatchingFromStorage()` in `home.component.ts`
  - [ ] 2.1 Add private `buildProgressData()` method that returns `Map<string, WatchProgressEntry>` keyed by `"${mediaType}:${id}"` (aggregating TV episodes to show-level by picking the most recent `updatedAt`):
    ```typescript
  ...
  - [ ] 2.2 Update `readContinueWatchingFromStorage()` to parse real localStorage data — remove the stub `return []`:
    ```typescript
  ...
  - [ ] 2.3 Change `continueWatchingItems` signal type from `signal<LibraryItem[]>` to `signal<ContinueWatchingItem[]>`:
    ```typescript
  ...
  - [ ] 2.4 Add `progressData` private signal — the Map used by `getProgressPercent()` and `isWatched()` for Recently Added and Library sections:
    ```typescript
  ...
  - [ ] 2.5 Add public `getProgressPercent(item: LibraryItem): number` method for template use:
    ```typescript
  ...
  - [ ] 2.6 Add public `isWatched(item: LibraryItem): boolean` method for template use:
    ```typescript
  ...
  - [ ] 2.7 Full updated import block (add `signal` if not already there — it already is):
    ```typescript
    import { Component, ChangeDetectionStrategy, computed, signal, inject } from '@angular/core';
    ```
    No new imports needed — all required imports were already present in 4-2.

- [ ] 3. Update `apps/frontend/src/app/home/home.component.html`
  - [ ] 3.1 Update the Continue Watching section poster cards:
  ...
  - [ ] 3.2 Update the Recently Added section poster cards:
  ...
  - [ ] 3.3 Update the Library (A-Z) section poster cards — same pattern as Recently Added:
    ```html
...
- [ ] 4. Update `apps/frontend/src/app/home/home.component.css`
  - [ ] 4.1 Add `position: relative` to `.poster-grid__image-wrap` (required for absolute-positioned progress bar):
    ```css
  ...
  - [ ] 4.2 Add `.poster-grid__progress` styles for the thin deep orange progress bar at poster bottom edge:
    ```css
    ...
    .poster-grid__progress {
      ...
      left: 0;
      height: 3px;
      background-color: var(--color-progress);  /* --color-progress = var(--color-accent) = #e65100 */
  ...
  - [ ] 4.3 Add `.poster-grid__image-wrap--watched` dimming styles for completed titles:
    ```css
...
- [ ] 5. Update `apps/frontend/src/app/home/home.component.spec.ts`
  - [ ] 5.1 Update imports to include new exported interfaces:
    ```typescript
    import { HomeComponent, WATCH_PROGRESS_KEY, WatchProgressEntry, ContinueWatchingItem } from './home.component';
    ```
  - [ ] 5.2 Add helper to build a WatchProgressRecord for test data:
    ```typescript
    ...
    ```
  - [ ] 5.3 The existing test `'should show Continue Watching section hidden even when localStorage has progress key'`
    ...
    ```
  - [ ] 5.4 Test: Continue Watching items link to `/play/:fileId` (not `/movie/:id` or `/show/:id`):
    ```typescript
    ...
    ```
  - [ ] 5.5 Test: progress bar renders in Library section for partially watched movie:
    ```typescript
    ...
    ```
  - [ ] 5.6 Test: no progress bar for fully watched (watched=true) movie in Library:
    ```typescript
    ...
    ```
  - [ ] 5.7 Test: watched movie gets dimmed class in Library:
    Already covered by 5.6 above — the `expect(...--watched)` assertion proves dimming.

  - [ ] 5.8 Test: no dimmed class for non-watched movie:
    ```typescript
    ...
    ```
  - [ ] 5.9 Test: most recent TV episode entry determines the show's Continue Watching slot:
    ```typescript
    it('should use most recent episode for TV show Continue Watching entry', async () => {
      ...
      const record: Record<string, WatchProgressEntry> = {
        ...
        'tv:1399:10': {
          position: 3600, duration: 3600, watched: true,
          updatedAt: now - 10000,
          mediaType: 'tv', id: 1399, title: 'Game of Thrones',
          posterUrl: null, year: 2011, fileId: 10,
        },
        // Newer episode (ep 2) — partially watched
        'tv:1399:20': {
          position: 600, duration: 3600, watched: false,
          updatedAt: now,
          mediaType: 'tv', id: 1399, title: 'Game of Thrones',
          posterUrl: null, year: 2011, fileId: 20,
        },
      };
      ...
      const cwSection = Array.from(el.querySelectorAll('section.library-section'))
        .find(s => s.querySelector('h2')?.textContent?.includes('Continue Watching'));
      const anchor = cwSection!.querySelector('a.poster-grid__item') as HTMLAnchorElement;
      expect(anchor.getAttribute('href')).toContain('/play/20');
    });
    ```
  - [ ] 5.10 Test: invalid/malformed localStorage entries are ignored without throwing:
    ```typescript
